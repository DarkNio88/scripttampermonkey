...
function log() {
    if ( DEBUG ) {
        ...
        // args.unshift('[LibreTranslate Translate Tooltip]');
...
function translate(text, left, top) {
    ...
    $tooltip.display(true).width('auto').find('.translation').text('Loading...').end();
    GM_xmlhttpRequest({
        method: "POST",
        url: urlt+"?" +
            $.param({
                source: GM_getValue('sl') || 'auto',
                target: GM_getValue('tl') || 'auto',
                format: "text",
                q: text
            }),
        onload: function (response) {
            var json, translation = [];
            try {
                json = JSON.parse(response.responseText);
            } catch (e) {
                log(e, response);
                $tooltip.find('.translation').text('Parsing JSON error').end().width('auto');
                return;
            }
...
$setting.find('select')
        ...
        .end().find('[name="ctrl"]').prop('checked', Boolean(GM_getValue('ctrl')))
        .change(function () {
